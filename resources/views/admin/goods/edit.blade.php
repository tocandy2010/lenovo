				...
				if(xhr.responseText === '0'){
					...
					if(xhr.responseText){
						...
						for(var j =0;j<=(imgs.length)-1;j++){
							showimg.innerHTML += '<img src = "../../../public/upload/goods/showimg/'+imgs[j]+'" width = "150px" height = "150px" onclick = "cancelupload(this)">';
							upimg.setAttribute('style','display:none');
							openimg.setAttribute('style','display:block');
						}
						showimg.innerHTML+="<input type = 'hidden' name = 'imgs' value = '"+imgs+"'>";
					}else{
						alert('操作失敗');
					}
				}
		...
		xhr.onreadystatechange = function(){
			if(this.readyState === 4 && this.status === 200){
				...
				if(pics === '0'){
					...
					if(pics){
					showpic.innerHTML = '<img src = "../../../public/upload/goods/showpic/'+pics+'" width = "150px" height = "150px" onclick = "cancelupload(this)">';
					uppic.setAttribute('style','display:none');
					openpic.setAttribute('style','display:block');
					showpic.innerHTML+="<input type = 'hidden' name = 'pics' value = '"+pics+"'>";
					}else{
						alert('上傳失敗');
					}
				}
