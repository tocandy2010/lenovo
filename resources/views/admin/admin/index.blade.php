...
<div>
	...
	<span>共<span id = 'count'>{{$count}}</span>位</span>
	</div>
<table border = "1">
	<tr>
		<th>id</th>
	...
	<tr id = '{{$v->id}}'>
		<td>{{$v->id}}</td>
		<td>{{$v->name}}</td>
		<td>{{substr($v->pass,0,15)."..."}}</td>
		<td>{{$v->time}}</td>
		<td>{{$v->lasttime}}</td>
		<td>{{$v->count}}</td>
		@if(!$v->status)
		<td><span class = 'btn btn-success' onclick = 'status(this,{{$v->id}},0)'>正常</span></td>
		@else
		<td><span class = 'btn btn-danger' onclick = 'status(this,{{$v->id}},1)'>停權</span></td>
		@endif
		<td>
		<a href = "javascript:;" data-toggle="modal" data-target="#edit" on onclick = 'edit({{$v->id}})'>修改</a>   
		<a href = "javascript:;" onclick = "deletes(this,{{$v->id}},{{"'".$v->name."'"}})">刪除</a>
		</td>
	</tr>
	@endforeach	
</table>
